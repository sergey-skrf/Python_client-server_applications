...
class Server:
    ...
    def process_client_message(message):
        ...
        if ACTION in message and\
            ...
            return {RESPONSE: 200}
        LOGGER.error(f'Неверный запрос от клиета : {message}')
        return {
            RESPONSE: 400,
            ERROR: 'Bad Request'
        }
    ...
    def main(self):
        ...
        try:
            ...
            LOGGER.error(f'В качастве порта может быть указано только число в диапазоне от 1024 до 65535.')
            sys.exit(1)
        ...
        try:
            ...
            LOGGER.error(f'После параметра \'a\'- необходимо указать адрес, который будет слушать сервер.')
            sys.exit(1)
