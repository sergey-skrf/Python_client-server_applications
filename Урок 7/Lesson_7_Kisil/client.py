...
class Client:
    ...
    def main(self):
        ...
        try:
            ...
            MessageProcessing.send_message(transport, Client.create_presence())
            answer = Client.process_response_ans(MessageProcessing.get_message(transport))
            ...
            while True:
                ...
                if client_mode == 'send':
                    try:
                        MessageProcessing.send_message(transport, Client.create_message(transport))
                ...
                if client_mode == 'listen':
                    try:
                        Client.message_from_server(MessageProcessing.get_message(transport))
                    except (ConnectionResetError, ConnectionError, ConnectionAbortedError):
                        LOGGER.error(f'Соединение с сервером {server_address} было потеряно.')
                        sys.exit(1)
